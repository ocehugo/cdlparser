...
# support python2 and python3 with past module from future package
...
# default fill values for netCDF-3 data types (as defined in netcdf.h
# include file)
NC_FILL_BYTE = np.int8(-127)
NC_FILL_CHAR = np.str_('\0')
NC_FILL_SHORT = np.int16(-32767)
NC_FILL_INT = np.int32(-2147483647)
# should get rounded to 9.96921e+36
NC_FILL_FLOAT = np.float32(9.9692099683868690e+36)
...
#-------------------------------------------------------------------------
    ...
    def __init__(
        ...
        self.cdlfile = None
        self.ncdataset = None
        # self.dryrun = kwargs.pop('dryrun', False)   # TODO: enable dry-run
        # option
        self.init_logger()
    ...
    def parse_file(self, cdlfile, ncfile=None):
        ...
        f = open(cdlfile)
        # FIXME: can we parse input w/o reading entire CDL file into memory?
        data = f.read()
        f.close()
    ...
    def parse_text(self, cdltext, ncfile=None):
        ...
        """
        self.ncfile = ncfile
        # if netcdf dataset handle exists, e.g. from previous parsing
        # operation, try to close it
        if self.ncdataset:
            try:
...
#-------------------------------------------------------------------------
    ...
    # this tells the parser which rule to kick off with (the p_ncdesc method
    # in this case)
    start = "ncdesc"
    ...
    # the full list of CDL tokens to parse - mostly named exactly as per the
    # ncgen.l file
    tokens = ['NETCDF',
              'DIMENSIONS',
              'VARIABLES',
              ...
              'LPAREN',
              'RPAREN',
              'EOL'] + list(set(reserved_words.values()))

    # literal characters
    literals = [',', ':']

    # Partially relaxed version of the UTF8 character set, and the one used in
    # the ncgen3.l flex file.
    ...
    idescaped = r"""\\[ !"#$%&'()*,:;<=>?\[\\\]^`{|}~]"""
    ID = r'([a-zA-Z_]|' + UTF8 + r'|\\[0-9])([a-zA-Z0-9_.@+-]|' + \
        UTF8 + r'|' + idescaped + r')*'
    ...
    float_const = r'[+-]?[0-9]*\.[0-9]*' + \
        exp + r'?[Ff]|[+-]?[0-9]*' + exp + r'[Ff]'
    double_const = r'[+-]?[0-9]*\.[0-9]*' + exp + \
        r'?[Dd]?|[+-]?[0-9]*' + exp + r'[Dd]?'
    byte_const = r"([+-]?[0-9]+[Bb])|" + \
                 r"(\'[^\\]\')|(\'\\.\')|" + \
                 r"(\'\\[0-7][0-7]?[0-7]?\')|" + \
    ...
    # TOKEN DEFINITIONS
    # Note that the t_xxx naming convention used below is a requirement of the
    # ply package.
    ...
    # numeric constants (order of appearance is extremely important and
    # differs from ncgen3.l file)
    @TOKEN(float_const)
    ...
    def t_SHORT_CONST(self, t):
        ...
        # r'[+-]?[0-9]+[sS]|0[xX][0-9a-fA-F]+[sS]'   # original regex in
        # ncgen3.l file
        try:
            int_val = int(eval(t.value[:-1]))
        except SyntaxError:  # py3 error
            try:
                int_val = int(eval(t.value[0]+'o'+t.value[1:-1]))
            except TypeError:
                errmsg = "Bad short constant: %s" % t.value
                raise CDLContentError(errmsg)
        except TypeError: # py2 error
                errmsg = "Bad short constant: %s" % t.value
                raise CDLContentError(errmsg)
    ...
    # mechanisms defined in ncgen3.l for decimal, octal and hex integer
    # constants.
    ...
    def p_dimdecl(self, p):
        ...
        if isinstance(p[3], basestring):
            if p[3] == "unlimited":
                ...
                raise CDLContentError(
                    "Unrecognised dimension length specifier: '%s'." %
                    p[3])
        else:
            dimname = p[1]
            dimlen = int(p[3])
            if dimlen <= 0:
                raise CDLContentError(
                    "Length of dimension '%s' must be positive." %
                    dimname)
        if dimname:
            self.curr_dim = self.ncdataset.createDimension(dimname, dimlen)
            unlim = " (unlimited)" if dimlen == 0 else ""
            self.logger.info(
                "Created dimension %s with length %s%s" %
                (dimname, dimlen, unlim))

    def p_dimd(self, p):
        """dimd : dim"""
        if p[1] in self.ncdataset.dimensions:
            raise CDLContentError(
                "Duplicate declaration for dimension '%s'." %
                p[1])
        p[0] = p[1]
    ...
    def p_varspec(self, p):
        ...
        self.curr_var = self.ncdataset.createVariable(
            p[1],
            self.datatype,
            dimensions=dims,
            shuffle=False)
        self.logger.info(
            "Created variable %s with data type '%s' and dimensions %s" %
    ...
    # attribute value. They cannot be prefixed with a type declaration, as is
    # possible at CDL v4.
    ...
    def p_avar(self, p):
        ...
        if self.ncdataset:
            if varname not in self.ncdataset.variables:
                raise CDLContentError(
                    "Variable %s is not defined or reference precedes definition." %
                    varname)
    ...
    def p_datadecl(self, p):
        ...
        if self.ncdataset:
            if p[1] not in self.ncdataset.variables:
                raise CDLContentError(
                    "Variable %s referenced in data section is not defined." %
                    p[1])
            ...
            try:
                ...
                self.logger.info(
                    "Wrote %d data value(s) for variable %s" %
                    (len(arr), p[1]))
    ...
    def p_constlist(self, p):
        """constlist : constlist ',' dconst
                     | dconst"""
        # FIXME: repeatedly appending values to a list will be inefficient for
        # large data arrays
        if len(p) == 2:
            p[0] = p[1:]
        else:
    ...
    def p_const(self, p):
        ...
        if p[1] == FILL_STRING:
            ...
            if self.curr_var is not None and self.curr_var.dtype.kind != 'S':
                ...
                self.logger.warn(
                    "Unable to replace fill value. Check CDL input for possible errors.")
                p[0] = p[1]
        else:
            p[0] = p[1]
    ...
    # TODO: consider adding a '_' prefix to these methods to make them
    # pseudo-private.
        ...
        # variable-scope attribute
            ...
            try:
                ...
                self.logger.info(
                    ...
                    (varname, attname, repr(attval)))
            except:
                raise CDLContentError(
                    "Invalid attribute name specification: '%s'" %
                    attid)
    ...
    def write_var_data(self, var, arr):
        ...
        if is_scalar:
            try:
                ...
                self.logger.debug(
                    "Assigned value %r to scalar variable %s" %
                    (arr[0], var._name))
            except:
        ...
        # for char-valued variables we need to divide by the length of the last
        # dimension
        arrlen = len(arr)
        varlen = var.size
        ...
        # length of record dimension is 0, assume that total variable length =
        ...
        if is_recvar:
            ...
            if rec_dimlen > 0:   # record dimension has been set to non-zero
                ...
                reclen = 1
                if var.ndim > 1:
                    reclen = reduce(lambda x,
                                    y: x*y,
                                    [x for x in var.shape if x > 0])
                self.logger.debug("Expected length of variable = %d" % varlen)
            # check that reclen is integer factor of variable length
            if varlen % reclen != 0:
                errmsg = "Record length %d is not a factor of variable length %d" % (
                    reclen, varlen)
                raise CDLContentError(errmsg)
            self.logger.debug("Length of one data record = %d" % reclen)

        # pad out data array with fill values if too few values were defined in
        # the CDL source
        if arrlen < varlen:
            pad_array(var, varlen, arr)
            self.logger.info(
                "Padded input data array with %d fill values" %
                (varlen-arrlen))
            arrlen = len(arr)
...
#-------------------------------------------------------------------------
    ...
    try:
        # wrap in try block since dataset may get closed by parser
        ncdataset.close()
    except:
        pass
